...
class NISTASD(object):
    # Taken from: http://intermittentshorts.blogspot.de/2012/12/nist-atomic-spectral-database-parser.html
...
class NISTLines(HasTraits):
    spec = Str('O')     # Species
    lowwl = Float(1)    # lower limit for get_lines()
    uppwl = Float(30)   # upper limit for get_lines()
    order = Int(1)      # wavelength scaling, 1 equals to nm as given by the NIST database
    lines = List        # List of dictionaries with line information

    nistasd_obj = Instance(NISTASD) # Instance of the Downloader
    plot_btn = Button('Plot Lines')
    print_information_btn = Button('Print All')

    def _plot_btn_fired(self):
        self.plot_lines()
        plt.show()
    ...
    def get_lines(self):
        ...
        filename = 'nist_data_' + self.spec + '.pkl'
        if self.nistasd_obj is None:
            ...
            if not os.path.isfile(direc + filename):
                print(self.__class__.__name__, ": found no spectrum in ", direc, ". Downloading spectra ...")
                tmp_nistasd = NISTASD(self.spec, 0.01, 10000., self.order)
    ...
    def plot_nist_lines_to_axis(self, axis, normalize_max=None, legend=True):
        ...
        # datacursor(lines)
        print(self.__class__.__name__, ": plotting ", len(lines), " lines of ", len(self.lines), " in total for ", self.spec, " from ", self.lowwl, " to ", self.uppwl, " nm")
    ...
    traits_view = View(
        VGroup(
            VGroup(
                Item('spec', label='element', width=-50),
                Item('lowwl', label='lower wavelength', width=-50),
                Item('uppwl', label='upper wavelength', width=-50),
                show_border=True,
                label='NIST emission lines',
            ),
            HGroup(
                UItem('plot_btn'),
                UItem('print_information_btn')
            ),
        ),
    )
...
if __name__ == '__main__':
    # Example 1
    nist = NISTLines(spec='Xe', lowwl=2, uppwl=20, order=1)
    nist.get_lines()
    print(nist.lines)
